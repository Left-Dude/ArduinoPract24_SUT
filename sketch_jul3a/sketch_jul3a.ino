...
void loop() {
  ...
  if (Serial1.available()) {
    ...
    DeserializationError error = deserializeJson(inJson, jsonString);
...
void analogExchange() {
  float lastConcentration1 = 0.0; // Переменная для хранения предыдущего значения концентрации
  float lastConcentration2 = 0.0; // Переменная для хранения предыдущего значения концентрации
  ...
  float error1 = concentration1 - lastConcentration1;
  float error2 = concentration2 - lastConcentration2;

  // Сохранение погрешности в JSON
  doc["error1"] = error1;
  doc["error2"] = error2;

  // Обновление предыдущих значений концентрации
  lastConcentration1 = concentration1;
  lastConcentration2 = concentration2;
...
void calcUnits(JsonDocument dds) {
  ...
  // }
  int size = dds["calibrationConcentrations"].size();
  int random_value_1 = random(0,size);
  int random_value_2 = random(0,size);
  int random_value_3 = random(0,size); 
